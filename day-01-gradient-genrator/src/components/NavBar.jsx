...
const NavBar = () => {
    const { view, setView, gradientType, setGradientType } = useContext(ViewContext)
    return (
        <>
            <nav className='w-full py-4 px-20 bg-white items-center flex justify-between shadow sticky top-0 z-10'>
                <h1 className='text-2xl font-semibold text-blue-600 cursor-pointer ' onClick={() => {
                    if (view !== 'card') {
                        setView('card')
                    }
                ...
                <div className='flex gap-6 align-centre'>
                    ...
                    <button onClick={() => {
                        setView('grid')
                    }} className={`px-4 py-2 rounded-xl ${view === 'grid' ? 'bg-blue-600 text-white hover:bg-blue-700 transition' : 'bg-gray-200 hover:bg-gray-300 transition'} cursor-pointer`}>Grid</button>
                </div>
            </nav>
        </>
    )
